  ...
  init {
    ...
    describe("/analysis/topic/accuracy") {
      it("should return accuracy percentage for last trained model and non trained model should be ignored") {
        trainingDao.insert(Training(1, TOPIC_MODEL_NAME, true, Timestamp.from(OffsetDateTime.now().toInstant()), Timestamp.from(OffsetDateTime.now().toInstant()), 42.1.toBigDecimal()))
